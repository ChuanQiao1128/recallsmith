...
1. 把“最核心的 3 条规则”写得更死一点

否则将来自己容易迷糊。建议在文档最前面再加一小节 Summary，直接写成三条： 1. Starter Deck（3 个）：
• 所有模式（TRIAL / FREE / PREMIUM）→ 全部卡片永久可用 + 完整间隔复习 + 完整日历（7~14 天）。 2. Paid Deck：
• TRIAL / PREMIUM → 全卡可用 + 完整记忆系统；
• FREE → 每个 Deck 只开放前 free_card_count（默认 50） 题，后面的卡全部锁。 3. 用户模式：
• 新注册 14 天 = TRIAL_FULL → 等同 PREMIUM；
• 14 天后无订阅 = FREE → 走上面 Starter + 前 50 卡规则。

这三条会成为以后所有实现/讨论的“最高优先级规则”。

⸻

2. 试用期结束后，Paid Deck 已经学过 50+ 卡时怎么表现？

场景：
用户在试用期里刷了 C# Deck 的 200 题；
14 天结束 → FREE 模式 → 你现在只开放前 50 题。

这里要在文档里讲明白 UX：
• 后端逻辑：
• 试用结束时，不删除任何进度数据；
• 只是：FREE 模式下，接口不再返回第 51 题及以后。
• 前端展示建议：
• Deck 卡片列表里：
• 仍然显示 “已学习 200/300 题”，但其中 50 题可访问，150 题锁 ~；
• 日历视图：
• 对于锁住的卡片，不再排列复习任务（否则用户点了也打不开）。

你可以在文档里加一句类似：

当用户从 TRIAL_FULL 降级到 FREE 时，
已经学习过但超出 free_card_count 的卡片不会被物理删除，但是：
FREE 模式下相关卡片不再出现在复习队列中，并在 UI 中标记为“已锁定，订阅后恢复”。

这样将来不会纠结“要不要把多学的部分残忍清零”。

⸻

3. FREE 模式下，Paid Deck 的间隔重复要不要“阉割算法”？

文档里现在写的是：
• Paid Deck 前 free_card_count 这些卡：
• FREE 模式下“也可以加入记忆计划，但可选限制每日上限 / 规划范围”。

我建议你先在 V1 明确：算法本身不阉割，只在数量上限和日历视野上做限制，理由：
• 技术实现更简单：
• 不需要写两套“简版/完整版”算法；
• 所有卡片只是在“有/没有资格进入队列”和“能不能看日历”的层面区分。
• 用户感受也更一致：
免费的前 50 卡也是“真正的间隔重复”，只是在数量和内容上被限制。

可以把那段改成：

FREE 模式下，对 Paid Deck 的前 free_card_count 卡片：
使用与 PREMIUM 相同的间隔重复算法；
但会对每日新卡/复习卡数量施加上限，并在日历视图中只展示短期（如 1 ～ 2 天）安排。

⸻

4. Starter Deck 的日历视野：FREE 和 PREMIUM 要不要有区别？

现在文档写的是：
• Starter Deck：
• FREE 也可以看“未来 7 ～ 14 天完整安排”；
• Paid Deck：
• FREE 只能看今天 / 明天。

这里没有技术问题，但要心理预期一致：
• FREE 用户在 Starter Deck 上会有非常完整的记忆体验；
• PREMIUM 的“日历优势”主要体现在 Paid Deck 和 更长的视野 上。

如果你想让 Premium 的差异再明显一点，可以考虑：
• FREE：
• Starter Deck 日历视野 = 7 天；
• PREMIUM：
• Starter + Paid Deck 日历视野 = 30 天。

不改也行，只是建议你在文档里把“具体天数”写死（比如 FREE 7 天 / PREMIUM 30 天），避免以后两边说法不一致。

⸻

5. C#/.NET、AWS/Azure 要不要再多一点“超小 Starter 味道”？

你现在的设计是：
• Starter Deck = JS 基础、HTTP、全栈基础；
• C#/.NET、AWS/Azure 全部归为 Paid Deck，只有每个 Deck 前 50 题免费。

可以考虑加点“小糖果”（可做可不做）：
• 在 C#/.NET Deck 的前 50 题里，刻意安排：
• 10 题是真·入门基础；
• 40 题是实用面试题；
• 让免费用户在前 50 题里就感受到“这是有水准的题库”，而不是全是概念题。
• AWS/Azure 也类似：
让前 30 ～ 50 题覆盖到真实考证的感觉，而不是只有术语解释。

这不是规则层面的修改，只是你出题时要记得：
前 50 题 = 免费展示区，要非常“好吃”。

⸻

二、可以标记成 V2 再做的点（现在可以先“不实现”）

有一些东西在文档中写着，但完全可以在实现时先打注释：

1. FREE 模式下的“每日 Paid 复习上限”
   • 文档里有提：
   • FREE 用户每天最多多少张 Paid Deck 卡片；
   • 你可以在 V1 中先不实现这个上限：
   • FREE 用户：Paid Deck 前 50 题，用多少就多少；
   • 真正的“限制”已经由“只有 50 题”和“日历视野短”实现了。

→ 等你用户多了，或者需要更精细的控制，再打开这个开关。

2. FREE 模式下的“个人草稿 Deck”
   • 文档中定义了 FREE 用户可以有 1 个本地草稿 Deck；
   • V1 完全可以先把个人 Deck 相关功能全关掉：
   • TRIAL/FREE/PREMIUM 统一都不能创建个人 Deck；
   • 等 Authoring + Catalog + 记忆系统都稳定了，再做“个人 Deck”。

→ 这样你可以先只处理“官方题库 + 用户复习进度”这一个维度。

⸻

三、整体判断
• 策略 B + 3 个 Starter Deck 的组合是合理且有吸引力的；
• 对 Full‑Stack 面试这个场景来说，JS/HTTP/全栈基础永久免费，C#/.NET/AWS/Azure 收费，是很顺的；
• 真正需要注意的只是：
• 试用结束后“已经学过 >50 题”的 UX 表现；
• FREE 模式下对 Paid Deck 的“算法是否简化”问题（我建议不简化，只限制内容/视野）；
• 先把 V1 和 V2 的实现边界划清楚，避免一上来就卷太多细节。
