...
type Props = NativeStackScreenProps<RootStackParamList, 'CollectionMilestone'>;

export function CollectionMilestoneScreen({ navigation, route }: Props) {
  const { poolId, tier } = route.params;
  return (
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={['#070A1F', '#1E1B4B', '#FAF3E0']} style={styles.gradient}>
        <ScrollView contentContainerStyle={styles.container}>
          <Text style={styles.eyebrow}>Collection milestone</Text>
          <Text style={styles.title}>{tier} collection reached in {poolId}</Text>
          <Text style={styles.body}>This is the base phase-A ceremony for owned-card progress before Hall lands in phase B.</Text>
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Reward</Text>
            <Text style={styles.cardBody}>+3 to +10 free pulls depending on tier. Ceremony queueing comes later, but the milestone front-end is now reachable.</Text>
          </View>
...
export default CollectionMilestoneScreen;
