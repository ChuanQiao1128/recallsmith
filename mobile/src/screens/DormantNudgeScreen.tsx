...
import { DORMANT_FIXTURES } from '../mock/recovery';

type Props = NativeStackScreenProps<RootStackParamList, 'DormantNudge'>;

export function DormantNudgeScreen({ navigation }: Props) {
  return (
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={['#FAF3E0', '#F5F3FF']} style={styles.gradient}>
        <ScrollView contentContainerStyle={styles.container}>
          <Text style={styles.eyebrow}>Welcome back</Text>
          <Text style={styles.title}>{DORMANT_FIXTURES.daysAway} days away</Text>
          <Text style={styles.body}>{DORMANT_FIXTURES.lastSessionSummary}</Text>

          <View style={styles.panel}>
            <Text style={styles.panelTitle}>Re-entry guidance</Text>
            <Text style={styles.panelBody}>Start with a short route today. If the schedule feels too dense, Fresh Start stays available as a softer reset.</Text>
          </View>

          <Pressable style={styles.primaryButton} onPress={() => navigation.navigate('Home')}>
            <Text style={styles.primaryButtonText}>Continue</Text>
          </Pressable>
          <Pressable style={styles.secondaryButton} onPress={() => navigation.navigate('FreshStartLanding')}>
...
export default DormantNudgeScreen;
