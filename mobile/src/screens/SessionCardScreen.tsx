...
import { buildRatedSessionState, buildSessionProgressVM, type CurrentCardLike } from '../features/gacha/session/sessionReviewHelpers';
...
function buildSessionRouteNodes(limit: number): RoutePreviewNode[] {
  ...
  return Array.from({ length: total }).map((_, index) => {
    ...
    if (role === 'warmup') {
      return { id: `warmup-${index}`, role, title: 'Warm-up node', subtitle: 'Start with one low-friction recall win.' };
    }
    if (role === 'elite') {
      return { id: `elite-${index}`, role, title: 'Elite recall', subtitle: 'A sharper mid-run check.' };
    }
    if (role === 'boss') {
      return { id: `boss-${index}`, role, title: 'Boss check', subtitle: 'Close the run with a clean recap test.' };
    }
    return { id: `normal-${index}`, role, title: 'Normal node', subtitle: 'Standard learning / recall step.' };
  });
}
...
export function SessionCardScreen({ navigation, route }: Props) {
    ...
    useCallback(() => {
      ...
      async function load() {
        ...
        try {
          ...
          const plannedRouteNodes = buildSessionRouteNodes(sessionLimit > 0 ? sessionLimit : Math.max(1, cardIndexRef.current?.cards.length ?? 1));
          startSession({
            sessionId: `${deckForStudy.Slug}-${now.getTime()}`,
            slug: deckForStudy.Slug,
            route: plannedRouteNodes,
  ...
  async function handleRating(uiRating: UiRating) {
    ...
    try {
      ...
      recordSessionRating({ stableUid: current.card.StableUid, rating: uiRating });
      advanceSession();
      ...
      if (!nextState.nextCurrent) {
        if (completionRoute === 'settlement') {
          navigation.replace('Settlement', {
            ...
            masteredCount: Math.max(0, nextState.updatedProgress.filter((item) => item.stage >= 4).length - progress.filter((item) => item.stage >= 4).length),
          });
          return;
        }
