...
vi.mock('../../src/features/gacha/session/sessionReviewHelpers', () => ({
  ...
  buildSessionProgressVM: vi.fn(() => ({ subtitle: 'Session 0/4 · Review Due' })),
  modeLabel: vi.fn(() => 'Review Due'),
}));
...
vi.mock('../../src/features/gacha/components/SessionProgressHeader', () => {
  const React = require('react');
  return {
    __esModule: true,
    default: () => React.createElement('View', null, React.createElement('Text', null, 'SessionProgressHeader')),
  };
});

vi.mock('../../src/features/gacha/components/RatingBar', () => {
  const React = require('react');
  return {
    __esModule: true,
    default: () => React.createElement('View', null, React.createElement('Text', null, 'RatingBar')),
  };
});
...
describe('ReviewScreen', () => {
  ...
  it('routes completed sessions to SessionSummary from the done state', async () => {
    const navigation = {
      navigate: vi.fn(),
      goBack: vi.fn(),
      replace: vi.fn(),
    ...
    act(() => {
      findPressableByLabel(tree, 'View summary').props.onPress();
    });
