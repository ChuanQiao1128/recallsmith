...
public sealed class PublicFunction
{
  ...
  public async Task<APIGatewayProxyResponse> Handler(JsonElement evt)
  {
    ...
    try
    {
      ...
      {
        return await Public.Billing.Verify.HandleBillingVerify(req, res, auth);
      }
      if (req.Path == "/api/v1/billing/webhook/apple")
      {
        return await Public.Billing.WebhookApple.HandleWebhookApple(req, res);
      }
      if (req.Path == "/api/v1/billing/webhook/google")
      {
        return await Public.Billing.WebhookGoogle.HandleWebhookGoogle(req, res);
      }

      // AI
      if (req.Path == "/api/v1/ai/explain-card")
      {
        return await Public.Ai.ExplainCard.HandleAiExplainCard(req, res, auth);
      }

      // Cognito admin
      if (req.Path == "/api/v1/admin/cognito/users")
      {
        return await Public.CognitoAdmin.Users.HandleCognitoUsers(req, res, auth);
      }

      var pDisable = RouteMatcher.Match("/api/v1/admin/cognito/users/:username/disable", req.Path);
      if (pDisable is not null)
      {
        return await Public.CognitoAdmin.DisableUser.HandleCognitoDisableUser(req, res, auth, pDisable);
      }

      var pDelete = RouteMatcher.Match("/api/v1/admin/cognito/users/:username/delete", req.Path);
      if (pDelete is not null)
      {
        return await Public.CognitoAdmin.DeleteUser.HandleCognitoDeleteUser(req, res, auth, pDelete);
      }
