...
**2026-09-22 更新（2.1.1 的代码侧已落地，PR `fix/api-jwt-verification`）。** `Auth.cs` 不再有"没有 authorizer 上下文就直接解码 payload"的路径。现在的顺序是：(1) `requestContext.authorizer.jwt.claims` 存在 ⇒ 照旧只信它；(2) 否则取 `Authorization: Bearer` 在进程内验签——`CognitoJwtVerifier`（`src_C/Shared/RecallSmith.Lambda.Common/JwtVerifier.cs`，Microsoft.IdentityModel.Tokens + System.IdentityModel.Tokens.Jwt 8.23.0）：RS256 对 issuer 的 JWKS 验签、`iss` ∈ 允许集（`AUTH_ISSUERS`，默认就是今天的两个 pool `_4Vf8uCXKt` 和 `_04hd6iisb`）、`exp`/`nbf` 允许 60 s 偏差、`token_use` ∈ {access, id}；alg=none、错 key、过期、外部 issuer、未知 kid、JWKS 取不到 ⇒ 一律拒绝（fail closed），`Require*` 三道门对被拒的 token 回 401 `UNAUTHORIZED`（同一 envelope），并打一行 `{tag:"auth", reason}` 结构化日志（不含 token）；(3) 旧的无验证解码只剩 `AUTH_ALLOW_UNVERIFIED=1` **且** `API_ENV != production` 时可用，启用时冷启动打 warn。JWKS 每 issuer 进程内缓存一份，未知 kid 触发一次刷新（刷新失败或仍未命中后 30 s 内不再刷），HTTP 取 3 s 超时。因为 core-vpc 在没有出口的 VPC 里（§1 表），两个 pool 的公钥随 zip 打包（`Shared/RecallSmith.Lambda.Common/Jwks/<poolId>.json`，`BundledJwks` 冷启动先读它），运行时不需要出网也能验签；pool 若重建需重新 curl 一次。测试：`JwtVerifierTests`（进程内 RSA + stub JWKS，覆盖上面每一条拒绝和"未知 kid 恰好刷新一次"）与 `AuthBearerTests`（伪造 super_admin 打真实路由 `GET /api/v1/authoring/cards` 走 `VpcFunction.Handler` ⇒ 401；真签名 ⇒ 200）。**网关侧的 JWT authorizer 与路由拆分（上表 2.1.1 的 (1)(2)）仍留给 infra wave**；它们落地后 (1) 路径会先命中，这段进程内验签只剩兜底作用。`src_C/Common/` 是 2025 年留下的旧副本，不在 sln 里、不参与打包，未改。
