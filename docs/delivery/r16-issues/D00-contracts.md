...
**Amendment 2026-09-22 (post-merge review of `e247585`, branch `fix/wave-d-review`, row 6).** `McqVerdictInput` gains one optional field appended after `hardStreak`: `redeal?: boolean;` — `attemptIndex >= 1`, i.e. a same-run redeal after a lapse. `again` drops the stage by two (`model.ts`), so a card that was at stage ≥ 3 comes back ten minutes later at stage ≥ 1 and, fast + sure + unchanged, would satisfy row 6 and undo the lapse it just recorded; plan §5.4 intends "重发永远到不了 easy" but only argued it for stage 0. Rule: `redeal === true` caps the result at `'good'` (row 6 → row 7); it changes nothing else (every non-row-6 input answers the same with or without the flag). `SessionCardScreen.settleMcq` passes `redeal: mcqState.attemptIndex >= 1`. New invariant (fast-check, `mcqVerdict.spec.ts`): redeal ⇒ ≠ easy; title `'a same-run redeal never reaches easy'`. The seven-row table test is unchanged (its base input has no `redeal`).
...
**Amendment 2026-09-22 (post-merge review of `e247585`, branch `fix/wave-d-review`, items 4–6).**
- *Wrong-unpicked rows* (`McqReviewBody`): the row no longer carries `opacity: 0.55` (that blended `inkSoft` to `#91867A` on the `softCream` card, 3.29:1 — below AA). The faded reading moves to the option text, `colors.inkSecondary` `#5A4B38` on `#FCF5EA` = 7.76:1 (style `optionTextMuted`), and to the letter disc alone (`letterDiscDimmed`, `opacity: 0.55`; the letter is also spoken in the row's accessibility label). Every other row keeps `inkSoft`.
- *Over-limit tap* (choose-two/three): the body's inline hint `mcq-over-limit-hint` still clears on the next change of picks or stage and now ALSO clears on its own after `MCQ_OVER_LIMIT_HINT_MS = 1_500` (`mcqConstants.ts`); the VoiceOver announcement `mcqOverLimitAnnouncement(n)` = `` `Pick ${n} answers — deselect one first` `` is the PARENT's job in `onOverLimit` (`SessionCardScreen.handleOverLimit`, through the existing `readRN('AccessibilityInfo', null)` lookup), the same split as the verdict announcement — the three components keep their single pinned `react-native` import line and never spell `AccessibilityInfo`.
- *"Why not?" toggles*: each `mcq-why-toggle-<key>` carries `accessibilityLabel={mcqWhyNotA11yLabel(index)}` = `` `Why not option ${mcqLetter(index)}` `` (letter by DISPLAYED position, never by key) next to `accessibilityState={{ expanded }}`, so VoiceOver no longer reads five identical buttons.
- `McqCoachLine`'s band gains `marginBottom: spacing.xs` because it is now mounted inside the rating dock (§2.5.3 amendment).
...
**Amendment 2026-09-22 (post-merge review of `e247585`, branch `fix/wave-d-review`, item 1 — MAJOR).** The coach line does NOT render "directly above the dock" as an in-flow sibling: `styles.ratingDock` is `position: 'absolute', bottom: 0` and opaque, so on a real layout it painted over the band and "Got it" was unreachable, after which `handleMcqNext` marked the hint seen forever. `<McqCoachLine visible={renderAsMcq && coachSeen === false} …/>` is now the FIRST child of the `review-rating-dock` View, above `McqActionDock` / `RatingBar`, in all three stages. The dock View carries `onLayout={(event) => setDockLayoutHeight(event.nativeEvent.layout.height)}` into `const [dockLayoutHeight, setDockLayoutHeight] = useState<number | null>(null)`; the ScrollView's `paddingBottom` is `dockLayoutHeight ?? ratingDockHeight` — the measured dock once it has laid out, the pinned stage default `(renderAsMcq ? MCQ_DOCK_HEIGHT : 164) + Math.max(insets.bottom, 8)` until then. `onOverLimit` is `handleOverLimit` (haptic Warning + the announcement of the §2.4 amendment); `settleMcq` passes `redeal` (§2.2 amendment). Tests (`session-card-mcq.screen.test.tsx`): the coach case asserts `mcq-coach-line` and `mcq-coach-dismiss` are descendants of `review-rating-dock`, not of `screen-session-card-primary-surface`, and precede `review-rating-bar`; new title `'reserves scroll padding for the measured dock height'` (default 224 → `onLayout` 301 → after dismiss 120); the redeal case now serves a stage-3 card whose redeal comes back at stage 1 and asserts `rating: 'good'`, never `'easy'`; the choose-two case asserts one `announceForAccessibility('Pick 2 answers — deselect one first')` (the suite's factory mock gains an `AccessibilityInfo` entry for that). The untouched Q/A suite still finds exactly one `review-rating-dock` View.
...
**Amendment 2026-09-22 (post-merge review of `e247585`, branch `fix/wave-d-review`, items 3 and 7).** *DrawResult*: "beside the rarity chip" is a COLUMN, not a second corner — the mark used to be pinned top-right at the rarity chip's y and the two grew into each other at Dynamic Type ≥ 1.2×. The rarity chip and the `draw-result-featured-kind` mark are the two in-flow children, in that order, of `<View testID="draw-result-featured-marks" style={localStyles.featuredMarkStack}>` (`position: 'absolute', left: 8, top: 8`, column, `gap: 4`) inside the art window; neither child positions itself; both Texts carry `numberOfLines={1} maxFontSizeMultiplier={1.3}` (this screen is a summary face, not the study surface — the §2.4 "no maxFontSizeMultiplier" rule is the MCQ components' only). The topic chip keeps its bottom-left corner. Test title: `'stacks the kind mark under the rarity chip so they never overlap at fontScale 1.5'` (layout props, not pixels). *Library tile*: `KIND_MARK_STYLE` is `fontSize: 11, lineHeight: 14` in `colors.inkSecondary` (`#5A4B38` on the tile's `softCream` `#FCF5EA` = 7.76:1; the old 9 pt `inkMuted` was 3.79:1), same slot between the icon and the question; `libraryMcqMark.test.tsx` pins `fontSize >= 11`, the colour and the icon → `MC` → question order.
