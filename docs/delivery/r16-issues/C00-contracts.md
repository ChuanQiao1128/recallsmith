...
4. **The seed reads storage, not only the caller's snapshot (adversarial probe S8, same day).** `settleRatingReward` seeded from `progressBefore`, which is `SessionCardScreen`'s in-memory `progress` state, refreshed only at load (`:352`) and after its own ratings (`:505`). A remote pull that lands mid-session (app_foreground, an empty-outbox background sync) merges into storage behind the screen's back; a seed taken from the stale snapshot backfilled only what the snapshot knew, decision 1's marker made it permanent, and every later-arriving learned card paid R1 on its next hard+ (99 of 99 in the probe). Decision (`mobile/src/features/gacha/rewards/storageFreshLearned.ts`, new, non-frozen): on the SEED path only (marker absent), the backfill is the **union** of (a) the learned uids of `progressBefore` and (b) the storage-fresh view — the stored progress key for the slug (`readStoredDeckProgress`, a new read-only export of `review/storage.ts` that reads the key `loadDeckProgress` reads through the same entry normalizer, with none of its writes) plus every row with `lastReviewedAtMs > 0` in the per-user remote cache `sync:remoteCache:v1:<slug>` that the frozen `progressSync.ts` stores verbatim from the server (the rule of `mergeRemoteIntoLocalProgress`, `:1133-1135`). A card learned in either view is backfilled as `0`; the superset is the safe direction. The cache is not redundant with the progress key: `handleRating` writes the whole in-memory array back with `saveDeckProgress` (`:451`) before it settles (`:452`), so a mid-session merge into the progress key has already been overwritten by the stale snapshot when the seed runs, and the cache is where the pulled rows still are until the next load replays it. `applyCachedRemoteProgress` + `loadDeckProgress` — the screen's own load path — were not called from the reward module: both are frozen, the import graph (`expo-crypto`, `expo-constants`, `react-native`, `apiClient`, `deckRepository` → `aws-amplify`, `expo-file-system`) would land in every suite that imports `sessionRewards`, the first needs a live token and a `DeckExport` from the filesystem, and it writes progress. The CURRENT card is exempt from view (b)'s stored-progress half — that half already holds this rating (the `:451` save), so trusting it would mean the first rating on a freshly seeded partition never pays — and is decided by `progressBefore` plus the remote-cache half, which this rating cannot have reached yet (it gets there only through a later push and pull), so a cache row for it means the account learned it before this rating. `seedNewCardLedgerIfAbsent` (§2.2) gains an optional trailing `extraLearnedUids?: () => Promise<Iterable<string>>`, invoked only when the marker is absent; its throw is a seed-time storage error (zero step, `skipped: 'storage-error'`, nothing written). A marker-present rating never reads the progress key or (beyond decision 2's settled gate) the cache. Repro promoted to `mobile/tests/unit/ledgerSeedStaleSnapshot.test.ts` (session opens unsettled or with a stale snapshot, the pull lands before the next rating, the first settled rating backfills all 99 account-learned cards, 0 of them pay, a genuinely new card still pays once). Not fixed here, by scope: the screen's `:451` wholesale save still overwrites a mid-session merge in the progress key until the next load replays the cache (pre-existing, `SessionCardScreen.tsx` not in this branch's scope).
