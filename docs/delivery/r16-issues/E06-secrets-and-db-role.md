...
> **Rebase 2026-09-26 (supervisor): reuse the finished implementation.** A previous run of this issue produced a complete, test-green implementation at commit `90d5179` (local ref `keep/e06-143-prev`; 20 files; it was blocked only by gate problems that are now fixed and by live drift that was cleared on 2026-09-26 — prod `terraform plan` is empty). Start with `git cherry-pick 90d5179`. On a conflict in `docs/delivery/r16-issues/E06.verify.sh` keep the BASE version (`git checkout --ours` during the pick) — never edit this issue's own verify script. Then re-run the acceptance commands, regenerate `E06.plan-allow.json` from a fresh read-only plan if it differs, fix only what fails, and commit. Do not re-implement from scratch.
