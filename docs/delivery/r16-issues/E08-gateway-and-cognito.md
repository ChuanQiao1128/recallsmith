...
> **Rebase 2026-09-26 (supervisor, after the 09-23 throttle incident and PR #161):** the stage default throttles are the validated variables in `infra/modules/api/variables.tf` (`throttling_*` 200/400 for `$default`, `dev_throttling_*` 50/100 for `dev`). This issue does not change their values and must not write numeric literals into `default_route_settings`; it only adds `route_settings`. The verify script checks `var.` references and the unchanged live values in the plan. The mobile refresh-token change (AWS-08) is NOT part of this issue.

Put the two Cognito pools in front of the API at the gateway: a second JWT authorizer `cognito-jwt-mobile` (issuer = the mobile pool `ap-southeast-2_04hd6iisb`, audience = client `7agirr7f56r9k5p6v6o63al1on`) next to the adopted console authorizer `828ehi`; split `ANY /{proxy+}` by prefix so console routes (`/api/v1/authoring/*`, `/api/v1/admin/*`, the three `edge-public` prefixes, `$default`, the catch-all) demand a console token and mobile routes (`/api/v1/sync/*`, `/api/v1/draw-state/*`, `/api/v1/user/*`, `/api/v1/premium/*`, `GET /api/v1/me`, `GET /api/v1/entitlements`, the two `premium-url` routes) demand a mobile token, while `GET /health` and the two RevenueCat webhooks stay `NONE`; collapse the three identical core-vpc integrations into one; keep both stages' default throttles on the existing validated variables (unchanged values) and add lower per-route throttles on sync, draw-state and the webhooks; reserve 40 concurrent executions for `core-vpc` and 2 for `worker-lambda`; and harden the console pool: TOTP MFA `ON`, access/id tokens 1 h, refresh 30 d, localhost callbacks moved off the prod client into a new `console-dev` client. Pure Terraform in `infra/` — no code, no mobile, no frontend file changes; the worker plans, the supervisor applies.
...
# inside each of the two stage resources:
  default_route_settings {
    ...
    # 2026-09-26 rebase: stage defaults stay on the E04-incident variables (validation > 0).
    # $default keeps var.throttling_* (live 200 rps / 400 burst); dev keeps var.dev_throttling_* (50 / 100).
    # NEVER write literal numbers here — a literal 0/0 took prod down on 2026-09-23.
    throttling_burst_limit   = var.throttling_burst_limit      # dev stage: var.dev_throttling_burst_limit
    throttling_rate_limit    = var.throttling_rate_limit       # dev stage: var.dev_throttling_rate_limit
  }
